...
typedef struct {     // auxiliary data structure
    ...
    const bam_hdr_t *h;
    int min_mapQ, min_len, min_ins_len; // mapQ filter; length filter
...
static int read_bam(void *data, bam1_t *b) // read level filters better go here to avoid pileup
{
    ...
    if (ret < 0) return ret;
    if (b->core.isize >= aux->min_ins_len) {
        ...
        if (!(b->core.flag & BAM_FUNMAP)) {
            if ((int) b->core.qual < aux->min_mapQ) {
                ...
                if (aux->bed && !(b->core.flag & BAM_FUNMAP) &&
                    !bed_overlap(aux->bed, chr, b->core.pos, b->core.pos + tlen))
                    b->core.flag |= BAM_FUNMAP;
            }
            if (!(b->core.flag & BAM_FUNMAP) && b->core.n_cigar > 1 && aux->max_clip_len < INT_MAX) {
                ...
                if (op == BAM_CSOFT_CLIP || op == BAM_CHARD_CLIP)
                    clip_len += bam_cigar_oplen(cigar[b->core.n_cigar - 1]);
...
pileup2allele(const bam_pileup1_t *p, int min_baseQ, uint64_t pos, int ref, int trim_len, int is_lianti,
              int is_stranded) { // collect allele information given a pileup1 record
    allele_t a;
    int i;
        ...
        if (!(c->flag & BAM_FSUPP)) {
            ...
            if (c->flag & BAM_FPAIRED) {
                if (c->flag & BAM_FPROPER_PAIR)
                    a.lt_pos = c->flag & BAM_FREAD1 ? pos5 << 1 | is_rev : (pos5 + c->isize) << 1 |
...
static inline void print_allele(const bam_pileup1_t *p, int l_ref, const char *ref, int pos, int max_del,
                                int is_vcf) { // print the allele. The format depends on is_vcf.
    const uint8_t *seq = bam_get_seq(p->b);
    int i, rest = max_del;
...
static void count_alleles(paux_t *pa, int n, int qual_as_depth) {
    ...
    if (pa->n_cnt > pa->max_cnt) { // expand the arrays if necessary
        ...
        pa->raw_cnt = (int *) realloc(pa->raw_cnt,
                                      pa->max_cnt * sizeof(int)); // FIXME: this wastes RAM, but not a big deal
...
int main_pileup(int argc, char *argv[]) {
    ...
    while ((n = getopt(argc, argv, "r:q:Q:l:f:dvcCS:Fs:D:V:uyRMb:T:x:L:P:N:n")) >= 0) {
        ...
        else if (n == 'i') min_ins_len = atoi(optarg); // minimum insert length
    ...
    if (optind == argc) {
        ...
        fprintf(stderr, "    -i INT      minimum insert length [%d]\n", min_ins_len);
    ...
    for (i = 0; i < n; ++i) {
        ...
        data[i]->min_mapQ = i < n - n_lt ? mapQ
                                         : mapQ_lt; // set the mapQ filter (bulk and lianti samples may use different thresholds)
        data[i]->min_len = min_len;                  // set the qlen filter
        data[i]->min_ins_len = min_ins_len;           // set the insert length filter
    ...
    plp = (const bam_pileup1_t **) calloc(n,
                                          sizeof(const bam_pileup1_t *)); // plp[i] points to the array of covering reads (internal in mplp)
        ...
        if (aux.tot_dp) {
            ...
            for (i = tmp_n = aux.n_a = 0; i < n; ++i) {
                if (i < n - n_lt) { // non-Lianti samples
                    for (j = 0; j < n_plp[i]; ++j) {
                        a[aux.n_a] = pileup2allele(&plp[i][j], baseQ, (uint64_t) i << 32 | j, r, trim_len, 0,
                                                   is_stranded);
                        if (!a[aux.n_a].is_skip) ++aux.n_a;
                    }
                } else { // Lianti samples
                    for (j = 0; j < n_plp[i]; ++j) {
                        a[aux.n_a] = pileup2allele(&plp[i][j], baseQ_lt, (uint64_t) i << 32 | j, r, trim_len, 1,
                                                   is_stranded);
                        if (!a[aux.n_a].is_skip) ++aux.n_a;
                    }
                    if (aux.n_a > tmp_n)
            ...
            if (var_only && aux.n_alleles == 1 && a[0].hash >> 63 == 0)
                continue; // var_only mode, but no ALT allele; skip
            if (is_fa) { // FASTA output
                ...
                if (is_vcf) {
                    ...
                    printf("\t%c\t", ref && pos < l_ref + beg ? ref[pos - beg] : 'N'); // print a single reference base
                // print alleles
                ...
                for (i = k = 1; i < aux.n_a; ++i)
                    if (a[i].indel != a[i - 1].indel || a[i].hash != a[i - 1].hash) {
                        print_allele(&plp[a[i].pos >> 32][(uint32_t) a[i].pos], l_ref, ref, pos - beg, aux.max_del,
                                     is_vcf);
                        if (++k != aux.n_alleles) putchar(',');
                    }
                if (is_vcf && aux.n_alleles == 1 && a[0].hash >> 63 == 0)
                    putchar('.'); // print placeholder if there is only the reference allele
                // compute and print qual
                for (i = !(a[0].hash >> 63), qual = 0; i < aux.n_alleles; ++i)
                    qual = qual > aux.support[i] ? qual : aux.support[i];
                // print INFO
                shift = (is_vcf &&
                         a[0].hash >> 63); // in VCF, if there is no ref allele, we need to shift the allele number
                if (is_vcf) {
                    printf("\t%d\t.\tAMQ=", qual);
